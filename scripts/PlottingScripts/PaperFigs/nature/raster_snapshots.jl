...
layout_resolution = (1000,900)#(1000,1100)
fig = Figure(size = layout_resolution)

plot_axs = Matrix{Any}(undef, 5-1, length(year_vals))
...
Colorbar(fig[1,5], limits = netage_clims, colormap = Reverse(netage_cmap), size = cbar_size, 
...
# Plot NPC rasters
for i in 1:length(year_vals)
    CairoMakie.plot!(plot_axs[1,i], netage_rasters[i]./12, colormap = Reverse(netage_cmap), colorrange = netage_clims)
    CairoMakie.plot!(plot_axs[2,i], npc_rasters[i], colormap = npc_cmap, colorrange = clims)
    CairoMakie.plot!(plot_axs[3,i], access_rasters[i], colormap = access_cmap, colorrange = clims)
    CairoMakie.plot!(plot_axs[4,i], use_rasters[i], colormap = use_cmap, colorrange = clims)

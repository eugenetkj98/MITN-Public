...
Used for subnational plots in a given country
"""
...
model_nat_est_timeseries = load("outputs/coverage_timeseries/model_coverage.jld2")

# %% Get ISO List
ISO_list = intersect(model_nat_est_timeseries["filt_ISOs"], survey_pred_data.ISO)
n_countries = length(ISO_list)

# %% Set base tiling dimension
tiling_dim = (6,9) 
...
# %% Temporary response variable conversion
f_use(x) = 1.26813*x + 0.050331
inv_f_use(x) = (x - 0.050331)/1.26813

# %% Setup master plot for a given ISO based on number of admin1 subnationa regions
for ISO in ISO_list
    println("Making plots for $ISO")
    # Load Data model estimates data
    model_subnat_est_timeseries = load("outputs/coverage_timeseries/subnational/$(ISO)_model_coverage.jld2")

    # Get metadata
    admin1_names = model_subnat_est_timeseries["admin1_names"]
    admin1_area_ids = model_subnat_est_timeseries["admin1_area_ids"]
    n_admin1 = length(admin1_names)
    YEAR_START = model_subnat_est_timeseries["YEAR_LIST"][1]
    YEAR_END = model_subnat_est_timeseries["YEAR_LIST"][end]
    ...
    for i in 1:(tiling_dim[1]*tiling_dim[2])#n_admin1
    ...
    # %% Add a Master title
    Label(fig_npc[0,1:tiling_dim[2]], "Subnational NPC ($ISO)", fontsize = 30)
    Label(fig_access[0,1:tiling_dim[2]], "Subational Household Access ($ISO)", fontsize = 30)
    Label(fig_hh_use[0,1:tiling_dim[2]], "Subnational Household Use Rate ($ISO)", fontsize = 30)
    Label(fig_pop_use[0,1:tiling_dim[2]], "Subnational Population Use Rate ($ISO)", fontsize = 30)

    # %% For Plot NPC model Time series for each country
    for i in 1:n_admin1
        admin1_name = admin1_names[i]
        area_id = admin1_area_ids[i]

        ax = Axis(npc_subfigs_list[i][1,1], title = "$admin1_name",
                    xlabel = "Year", ylabel = "NPC",
                    xticks = YEAR_START:2:YEAR_END, xticklabelrotation=π/2)
        ylims!(ax, (-0.05,1.05))

        # Plot Survey observed values
        filt_data = survey_pred_data[(survey_pred_data.admin_id .== area_id),:]
        plot!(ax, YEAR_VALS[filt_data.monthidx], filt_data.npc, color = scat_col_1, markersize = scat_size,
                label = "Survey")

        
        # Plot BV model time series prediction and errors

        lines!(ax, YEAR_VALS,model_subnat_est_timeseries["bv_subnat_npc"][i,:,2], linewidth = lw, color = linecol_bv,
                label = "BV")

        # Plot MITN model time series prediction and errors
        lines!(ax, YEAR_VALS,model_subnat_est_timeseries["mitn_subnat_npc"][i,:,2], linewidth = lw, color = linecol_mitn,
                label = "MITN")
        band!(ax, YEAR_VALS, model_subnat_est_timeseries["mitn_subnat_npc"][i,:,1], model_subnat_est_timeseries["mitn_subnat_npc"][i,:,3],
    ...
    for i in 1:n_admin1
        admin1_name = admin1_names[i]
        area_id = admin1_area_ids[i]
        ax = Axis(access_subfigs_list[i][1,1], title = "$admin1_name",
        ...
        if !(ISO ∈ ["BWA","DJI","GNQ","ETH","SOM","SSD"]) # Temporary fix. Exclude some countries from plotting survey access. There shouldn't be any available, (problem with data summary code)
            plot!(ax, YEAR_VALS[filt_data.monthidx], filt_data.access, color = scat_col_1, markersize = scat_size,
                    label = "Survey")
        end

        # Plot MITN model time series prediction and errors
        lines!(ax, YEAR_VALS,model_subnat_est_timeseries["mitn_subnat_access"][i,:,2], linewidth = lw, color = linecol_mitn,
                label = "MITN")
        band!(ax, YEAR_VALS, model_subnat_est_timeseries["mitn_subnat_access"][i,:,1], model_subnat_est_timeseries["mitn_subnat_access"][i,:,3],
                color = linecol_mitn, alpha = error_alpha)
    ...
    for i in 1:n_admin1
        ...
        ax = Axis(hh_use_subfigs_list[i][1,1], title = "$admin1_name",
                    xlabel = "Year", ylabel = "Use",
                    xticks = YEAR_START:2:YEAR_END, xticklabelrotation=π/2)
        ylims!(ax, (-0.05,1.05))

        # Plot Survey observed values
        filt_data = survey_pred_data[(survey_pred_data.admin_id .== area_id),:]
        plot!(ax, YEAR_VALS[filt_data.monthidx], 2 .* filt_data.use, color = scat_col_1, markersize = scat_size,
                label = "Survey (HH)")
        ...
        lines!(ax, YEAR_VALS, 2 .* (inv_f_use.(model_subnat_est_timeseries["bv_subnat_use"][i,:,2])), linewidth = lw, color = linecol_bv,
                label = "BV")
        band!(ax, YEAR_VALS, 2 .* (inv_f_use.(model_subnat_est_timeseries["bv_subnat_use"][i,:,1])), 2 .* (inv_f_use.(model_subnat_est_timeseries["bv_subnat_use"][i,:,3])),
                color = linecol_bv, alpha = error_alpha)

        # Plot MITN model time series prediction and errors
        lines!(ax, YEAR_VALS, 2 .* model_subnat_est_timeseries["mitn_subnat_use"][i,:,2], linewidth = lw, color = linecol_mitn,
                label = "MITN")
        band!(ax, YEAR_VALS, 2 .* model_subnat_est_timeseries["mitn_subnat_use"][i,:,1], 2 .* model_subnat_est_timeseries["mitn_subnat_use"][i,:,3],
                color = linecol_mitn, alpha = error_alpha)

        # Add legend
        axislegend(ax, position = :lt, groupgap = 0)
    end
    ...
    for i in 1:n_admin1
        ...
        ax = Axis(pop_use_subfigs_list[i][1,1], title = "$admin1_name",
                    xlabel = "Year", ylabel = "Use",
                    xticks = YEAR_START:2:YEAR_END, xticklabelrotation=π/2)
        ylims!(ax, (-0.05,1.05))
        ...
        lines!(ax, YEAR_VALS, model_subnat_est_timeseries["bv_subnat_use"][i,:,2], linewidth = lw, color = linecol_bv,
                label = "BV")
        band!(ax, YEAR_VALS, model_subnat_est_timeseries["bv_subnat_use"][i,:,1], model_subnat_est_timeseries["bv_subnat_use"][i,:,3],
                color = linecol_bv, alpha = error_alpha)

        # Plot MITN model time series prediction and errors
        lines!(ax, YEAR_VALS, f_use.(model_subnat_est_timeseries["mitn_subnat_use"][i,:,2]), linewidth = lw, color = linecol_mitn,
                label = "MITN")
        band!(ax, YEAR_VALS, f_use.(model_subnat_est_timeseries["mitn_subnat_use"][i,:,1]), f_use.(model_subnat_est_timeseries["mitn_subnat_use"][i,:,3]),
                color = linecol_mitn, alpha = error_alpha)
    ...
    mkpath("output_plots/ITN coverage summaries/timeseries/subnational/")
    save("output_plots/ITN coverage summaries/timeseries/subnational/$(ISO)_npc.pdf", fig_npc)
    save("output_plots/ITN coverage summaries/timeseries/subnational/$(ISO)_access.pdf", fig_access)
    save("output_plots/ITN coverage summaries/timeseries/subnational/$(ISO)_hh_use.pdf", fig_hh_use)
    save("output_plots/ITN coverage summaries/timeseries/subnational/$(ISO)_pop_use.pdf", fig_pop_use)
end

# %% Calculate country aggregates of subnat RMSE
...
for i in 1:n_countries
    ...
    npc_filt_data = survey_pred_data[(survey_pred_data.ISO .== ISO).&
                                    (survey_pred_data.admin_level .== 1) .&
                                    (.!isnan.(survey_pred_data.npc)) .&
                                    (.!isnan.(survey_pred_data.bv_npc)) .&
                                    (.!isnan.(survey_pred_data.mitn_npc)),:]

    survey_pred_data[(survey_pred_data.ISO .== ISO).&
                                    (survey_pred_data.admin_level .== 1) .&
                                    (.!isnan.(survey_pred_data.npc)) .&
                                    (.!isnan.(survey_pred_data.bv_npc)) .&
                                    (.!isnan.(survey_pred_data.mitn_npc)),:]

    bv_npc_RMSE[i] = sqrt(sum(npc_filt_data.population.*((npc_filt_data.bv_npc .- npc_filt_data.npc).^2))/sum(npc_filt_data.population))
    mitn_npc_RMSE[i] = sqrt(sum(npc_filt_data.population.*((npc_filt_data.mitn_npc .- npc_filt_data.npc).^2))/sum(npc_filt_data.population))
    ...
    mitn_pop_use_RMSE[i] = sqrt(mean((hh_use_filt_data.mitn_use .- f_use.(hh_use_filt_data.use)).^2))
end

# Calculate RMSE of full national level dataset
npc_filt_data = survey_pred_data[(survey_pred_data.admin_level .== 1) .&
                                (.!isnan.(survey_pred_data.npc)) .&
                                (.!isnan.(survey_pred_data.bv_npc)) .&
                                (.!isnan.(survey_pred_data.mitn_npc)),:]
...
access_filt_data = survey_pred_data[(survey_pred_data.admin_level .== 1) .&
                                (.!isnan.(survey_pred_data.access)) .&
                                # (.!isnan.(survey_pred_data.bv_access)) .&
                                (.!isnan.(survey_pred_data.mitn_access)) .& 
...
use_filt_data = survey_pred_data[(survey_pred_data.admin_level .== 1) .&
                                (.!isnan.(survey_pred_data.use)) .&
                                (.!isnan.(survey_pred_data.bv_use)) .&
                                (.!isnan.(survey_pred_data.mitn_use)),:]
...
# %% Make national level plots of aggregated subnat RMSE
...
xlims!(ax_npc, 0, n_countries + 1)
ylims!(ax_npc, -0.01, 0.32)

### RMSE values
plot!(ax_npc, (1:n_countries).-sep, bv_npc_RMSE, markersize = scat_size,
        label = "BV", color = linecol_bv)
plot!(ax_npc, (1:n_countries).+sep, mitn_npc_RMSE, markersize = scat_size,
...
ax_access = Axis(subfig_access_rmse[1,1], title = "Household Access", titlesize = titlesize,
                ...
                xlabelsize = labelsize, ylabelsize = labelsize, 
                xticks = (1:n_countries, ISO_list))
xlims!(ax_access, 0, n_countries + 1)
ylims!(ax_access, -0.01, 0.42)

### RMSE values
plot!(ax_access, (1:n_countries).-sep, bv_access_RMSE, markersize = scat_size,
        label = "BV", color = linecol_bv)
plot!(ax_access, (1:n_countries).+sep, mitn_access_RMSE, markersize = scat_size,
        label = "MITN", color = linecol_mitn)
...
ax_hh_use = Axis(subfig_hh_use_rmse[1,1], title = "Household Use", titlesize = titlesize,
                ...
                xlabelsize = labelsize, ylabelsize = labelsize, 
                xticks = (1:n_countries, ISO_list))
xlims!(ax_hh_use, 0, n_countries + 1)
ylims!(ax_hh_use, -0.01, 0.42)

### RMSE values
plot!(ax_hh_use, (1:n_countries).-sep, bv_hh_use_RMSE, markersize = scat_size,
        label = "BV", color = linecol_bv)
plot!(ax_hh_use, (1:n_countries).+sep, mitn_hh_use_RMSE, markersize = scat_size,
        label = "MITN", color = linecol_mitn)
...
ax_pop_use = Axis(subfig_pop_use_rmse[1,1], title = "Population Use", titlesize = titlesize,
                ...
                xlabelsize = labelsize, ylabelsize = labelsize, 
                xticks = (1:n_countries, ISO_list))
xlims!(ax_pop_use, 0, n_countries + 1)
ylims!(ax_hh_use, -0.01, 0.42)

### RMSE values
plot!(ax_pop_use, (1:n_countries).-sep, bv_pop_use_RMSE, markersize = scat_size,
        label = "BV", color = linecol_bv)
plot!(ax_pop_use, (1:n_countries).+sep, mitn_pop_use_RMSE, markersize = scat_size,
        label = "MITN", color = linecol_mitn)
...
ax_hh_pop = Axis(subfig_hh_pop_rmse[1,1], title = "Household vs Population Use Performance", titlesize = titlesize,
                ...
                xlabelsize = labelsize, ylabelsize = labelsize, 
                xticks = (1:n_countries, ISO_list))
xlims!(ax_hh_pop, 0, n_countries + 1)
ylims!(ax_hh_pop, -0.02, 1.02)
...
plot!(ax_hh_pop, (1:n_countries).-sep, bv_transfer_ratios, markersize = scat_size,
        label = "BV", color = linecol_bv)
plot!(ax_hh_pop, (1:n_countries).+sep, mitn_transfer_ratios, markersize = scat_size,
        label = "MITN", color = linecol_mitn)
...
### Add legend
axislegend(ax_hh_pop, position = :rt, groupgap = 0)
...
ax_full = Axis(subfig_full_rmse[1,1], title = "Full Survey Data", titlesize = titlesize,
                ...
                yticks = (0:0.05:0.25))
ylims!(ax_full, -0.002, 0.26)

plot!(ax_full, (1:4) .- sep, [bv_full_npc_RMSE, bv_full_access_RMSE, bv_full_hh_use_RMSE, bv_full_pop_use_RMSE],
        markersize = scat_size, label = "BV", color = linecol_bv)
plot!(ax_full, (1:4 .+ sep), [mitn_full_npc_RMSE, mitn_full_access_RMSE, mitn_full_hh_use_RMSE, mitn_full_pop_use_RMSE],
...
save("output_plots/ITN Coverage summaries/subnat_aggregated_RMSE.pdf", fig_rmse)
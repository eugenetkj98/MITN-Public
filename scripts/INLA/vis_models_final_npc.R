...
for (year in start_year:end_year) {
  ...
  # Undo IHS transform
  # adj_pred <- pred # For npc
  # adj_pred <- inv_gap_emplogit(inv_ihs(pred, access_theta)) # For access
  ...
  save_filename_mean = str_glue("outputs/INLA/rasters/inla_logmodel_final_npc/final_NPC_logmodel_{year}_mean.tif")
  ...
  for (i in 1:n_samples_saved){
    ...
    save_filename_sample = str_glue("outputs/INLA/rasters/inla_logmodel_final_npc/final_NPC_logmodel_{year}_sample_{i}.tif")

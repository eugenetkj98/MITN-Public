...
# epsilon <- 1e-5
# 
# use_ratios_pred <- inv_gap_emplogit(inv_ihs(pred, use_theta))
# use_gap_pred <- use_ratios_pred*(inla_data$access)
# use_pred <- inla_data$access - use_gap_pred
...
RMSE <- round(sqrt(mean((inla_data$use - use_pred)^2)), digits = 4)
MAE <- round(mean(abs(inla_data$use - use_pred)), digits = 4)
CORR <- round(cor(inla_data$use, as.numeric(use_pred)), digits = 4)

# Make Fit Plot
title <- str_glue("Use Fit (Gap Model) Full Training Data\nMAE = {MAE}, RMSE = {RMSE}, CORR = {CORR}")
# title <- str_glue("Use Fit (Gap Model) 50% Training, Out of Sample \nMAE = {MAE}, RMSE = {RMSE}, CORR = {CORR}")

plot(inla_data$use, use_pred, 
     col = rgb(red = 0, green = 0, blue = 1, alpha = 0.1), cex=0.1,
     xlab = "Survey Use", ylab = "Fitted Use", main = title)
abline(a=0, b=1)

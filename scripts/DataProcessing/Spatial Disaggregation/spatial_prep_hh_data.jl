...
for ISO_i in ProgressBar(1:length(filt_ISOs))
    ...
    Threads.@threads for i in 1:n_unique_obs
        ...
        npc = sum(weights.*npc_entries)/sum(weights)
        access = sum(weights.*access_entries)/sum(weights)
        # use = sum(weights .* min.(2 .*filt_ISOs.n_itn_used, filt_ISOs.hh_size))/sum(weights .* filt_ISOs.hh_size) #sum(weights.*use_entries)/sum(weights)
...
cov_raster = replace_missing(cov_raster, missingval=NaN)
...
cov_raster_m = Raster(Rasters.aggregate(Rasters.Center(), cov_raster, downsampling_ratio; skipmissingval=true, progress=false))[:,:,1]
cov_raster_m = replace_missing(cov_raster_m, missingval = -9999)
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        cov_raster_filename = "NA"
        
        if month < 10
            cov_raster_filename = "EVI_v6.$(year).0$(month).max.5km.max.tif"
        else
            cov_raster_filename =  "EVI_v6.$(year).$(month).max.5km.max.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        if month < 10
            cov_raster_filename = "LST_Day_v6.$(year).0$(month).max.5km.max.tif"
        else
            cov_raster_filename =  "LST_Day_v6.$(year).$(month).max.5km.max.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        if month < 10
            cov_raster_filename = "LST_Night_v6.$(year).0$(month).max.5km.max.tif"
        else
            cov_raster_filename =  "LST_Night_v6.$(year).$(month).max.5km.max.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        if month < 10
            cov_raster_filename = "LST_DiurnalDiff_v6.$(year).0$(month).max.5km.max.tif"
        else
            cov_raster_filename =  "LST_DiurnalDiff_v6.$(year).$(month).max.5km.max.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        if month < 10
            cov_raster_filename = "TCW_v6.$(year).0$(month).max.5km.max.tif"
        else
            cov_raster_filename =  "TCW_v6.$(year).$(month).max.5km.max.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        if month < 10
            cov_raster_filename = "TSI-Martens2-Pf.$(year).0$(month).Data.5km.Data.tif"
        else
            cov_raster_filename =  "TSI-Martens2-Pf.$(year).$(month).Data.5km.Data.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    for month_idx in ProgressBar(1:length(months), leave = false)
        ...
        if month < 10
            cov_raster_filename = "TCB_v6.$(year).0$(month).max.5km.max.tif"
        else
            cov_raster_filename =  "TCB_v6.$(year).$(month).max.5km.max.tif"
        end
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-00_Unclassified.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-01_Evergreen_Needleleaf_Forest.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-02_Evergreen_Broadleaf_Forest.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-03_Deciduous_Needleleaf_Forest.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-04_Deciduous_Broadleaf_Forest.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-05_Mixed_Forest.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-06_Closed_Shrublands.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-07_Open_Shrublands.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-08_Woody_Savannas.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-09_Savannas.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-10_Grasslands.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-11_Permanent_Wetlands.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-12_Croplands.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-13_Urban_And_Built_Up.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-14_Cropland_Natural_Vegetation_Mosaic.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-15_Snow_And_Ice.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-16_Barren_Or_Sparsely_Populated.$(year).Annual.Data.5km.fraction.tif"
...
for year_idx in ProgressBar(1:length(YEAR_VALS))
    ...
    # Define raster filename
    cov_raster_filename = "IGBP_Landcover_Class-17_Water.$(year).Annual.Data.5km.fraction.tif"

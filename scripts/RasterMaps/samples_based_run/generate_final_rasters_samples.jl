...
#     for month in 1:12
        ...
        for sample_i in 1:n_samples
            ...
            pmodel_dep_sample_raster =  replace_missing(Raster(inla_dir*"inla_pmodel_deployment/DEP_pmodel_$(year)_sample_$(sample_i).tif"), missingval = NaN)
            ...
            deploy_map_sample_raster = inv_p_transform.(pmodel_dep_sample_raster, access_snf_sample_raster./(2 .* npc_snf_sample_raster), n=2)
            deploy_map_sample_raster[findall(npc_snf_sample_raster .== 0)].= 0

            println("Calculating sample Access raster...")
            access_map_sample_raster = min.(deploy_map_sample_raster.*(2 .*npc_map_sample_raster),1) 

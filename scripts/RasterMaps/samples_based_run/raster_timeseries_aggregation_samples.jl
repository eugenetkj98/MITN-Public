...
# %% Time bounds
YEAR_START = 2018#YEAR_NAT_START
YEAR_END = 2019#YEAR_NAT_END
...
for year in YEAR_START:YEAR_END
    ...
    for month in 1:12
        ...
        npc_sample_rasters = replace_missing.(Raster.(raster_dir*"final_npc/logmodel_npc/".*npc_sample_raster_filenames), missingval = NaN)

        access_sample_raster_filenames = "access_$(year)_$(month_str)_sample_".*string.(1:n_samples).*".tif"
        access_sample_rasters = replace_missing.(Raster.(raster_dir*"final_access/pmodel_access/".*access_sample_raster_filenames), missingval = NaN)

        use_sample_raster_filenames = "use_$(year)_$(month_str)_sample_".*string.(1:n_samples).*".tif"
        use_sample_rasters = replace_missing.(Raster.(raster_dir*"final_use/logis_use/".*use_sample_raster_filenames), missingval = NaN)

        util_sample_raster_filenames = "utilisation_$(year)_$(month_str)_sample_".*string.(1:n_samples).*".tif"
        util_sample_rasters = replace_missing.(Raster.(raster_dir*"final_utilisation/monthly/".*util_sample_raster_filenames), missingval = NaN)

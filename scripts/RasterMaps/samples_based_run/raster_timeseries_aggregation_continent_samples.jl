"""
Author: Eugene Tan
Date Created: 9/6/2025
Last Updated: 9/6/2025
Extract continent level population weighted mean time series for ITN metrics
"""
...
output_dir = OUTPUT_DIR*"coverage_timeseries/master_extractions_parts/continent/"
...
# Continent data
continent_npc_snf_vals = Vector{Float32}(undef, n_samples)
continent_access_snf_vals = Vector{Float32}(undef, n_samples)
continent_npc_raster_vals = Vector{Float32}(undef, n_samples)
continent_access_raster_vals = Vector{Float32}(undef, n_samples)
continent_use_raster_vals = Vector{Float32}(undef, n_samples)
continent_util_raster_vals = Vector{Float32}(undef, n_samples)
continent_eff_raster_vals = Vector{Float32}(undef, n_samples)

# Admin0 data
admin0_npc_snf_vals = Matrix{Float32}(undef, n_samples, length(filt_ISOs))
admin0_access_snf_vals = Matrix{Float32}(undef, n_samples, length(filt_ISOs))
...
for ISO_i in 1:length(filt_ISOs)
    ...
    admin1_snf_npc_vals = Matrix{Float32}(undef, n_admin1, n_samples)
    admin1_snf_access_vals = Matrix{Float32}(undef, n_admin1, n_samples)
    admin1_pop = Vector{Float32}(undef, n_admin1)

    # Scrape Admin1 data from SNF Draws
    Threads.@threads for admin1_i in 1:n_admin1
        println("Extracting SNF [$(ISO_i)/$(length(filt_ISOs))]: $(ISO), region $(admin1_i) of $(n_admin1)")

        # Calculate total population of admin1 region
        area_id = subnat_snf_post_draws["merged_outputs"][admin1_i]["area_id"]
        geometry = admin1_shapes_geoIO[findfirst(admin1_shapes_geoIO.area_id .== area_id),"geometry"]
        pop_mask = Rasters.trim(mask(population_raster, with = geometry); pad=0)
        admin1_pop[admin1_i] = sum(pop_mask[findall(.!isnan.(pop_mask))])

        # Generate random idx to draw from
        rand_idxs = rand(1:size(subnat_snf_post_draws["merged_outputs"][admin1_i]["ADJ_NPC_MONTHLY_TOTAL_samples"])[1], n_samples)

        # Stock and flow NPC values and CI
        admin1_snf_npc_vals[admin1_i,:] = subnat_snf_post_draws["merged_outputs"][admin1_i]["ADJ_NPC_MONTHLY_TOTAL_samples"][rand_idxs,monthidx]

        # Stock and flow Access values and CI
        admin1_snf_access_vals[admin1_i,:] = subnat_snf_post_draws["merged_outputs"][admin1_i]["ADJ_λ_ACCESS_samples"][rand_idxs,monthidx]
    end

    admin0_geometry[ISO_i] = admin0_shapes_geoIO[findfirst(admin0_shapes_geoIO.ISO .== ISO),"geometry"]
    admin0_npc_snf_vals[:,ISO_i] = ((admin1_pop')*admin1_snf_npc_vals)[:]./sum(admin1_pop)
    admin0_access_snf_vals[:,ISO_i] = ((admin1_pop')*admin1_snf_access_vals)[:]./sum(admin1_pop)
    admin0_pop_vals[ISO_i] = sum(admin1_pop)
end

continent_npc_snf_vals = (((admin0_pop_vals')*(admin0_npc_snf_vals'))./sum(admin0_pop_vals))[:]
continent_access_snf_vals = (((admin0_pop_vals')*(admin0_access_snf_vals'))./sum(admin0_pop_vals))[:]
...
for sample_i in 1:n_samples
    ...
    # Admin Raster Extraction
    println("Sample: [$(sample_i)/$(n_samples)], Continent Extraction")
    ...
    # Get population weighted mean
    continent_npc_raster_vals[sample_i] = aggregate_raster_weighted_mean(npc_sample_raster_masked, pop_masked)
    continent_access_raster_vals[sample_i] = aggregate_raster_weighted_mean(access_sample_raster_masked, pop_masked)
    continent_use_raster_vals[sample_i] = aggregate_raster_weighted_mean(use_sample_raster_masked, pop_masked)
    continent_util_raster_vals[sample_i] = (continent_use_raster_vals[sample_i]/2)/continent_npc_raster_vals[sample_i]
    continent_eff_raster_vals[sample_i] = (continent_use_raster_vals[sample_i])/(continent_access_raster_vals[sample_i])
end
...
pop_total = Float64(sum(admin0_pop_vals))

raster_npc_mean = Float64(mean(continent_npc_raster_vals))
raster_access_mean = Float64(mean(continent_access_raster_vals))
raster_use_mean = Float64(mean(continent_use_raster_vals))

npc_quantiles = Float64.(quantile(continent_npc_raster_vals, [0.025, 0.975]))
access_quantiles = Float64.(quantile(continent_access_raster_vals, [0.025, 0.975]))
use_quantiles = Float64.(quantile(continent_use_raster_vals, [0.025, 0.975]))
...
snf_npc_lower, snf_npc_upper = quantile(continent_npc_snf_vals, [0.025, 0.975])

snf_access_mean = Float64(mean(continent_access_snf_vals))
snf_access_lower, snf_access_upper = quantile(continent_access_snf_vals, [0.025, 0.975])

df_continent = DataFrame(ISO = "XXX", category = "Continent", 
                admin_name = "Africa", 
                area_id = 9999999999,
...
compiled_dataframe = vcat(df_continent)
mkpath(output_dir)
